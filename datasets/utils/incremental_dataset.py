...
class IncrementalDataset:
    ...
    def __init__(self, args: Namespace) -> None:
        ...
        self.keep_ratio = args.keep_ratio           # sample ratio
...
def store_masked_loaders(train_dataset: datasets, test_dataset: datasets,
    ...
    # if only keep partial data
    if setting.keep_ratio != 1.0:
        num_samples_to_keep = int(len(train_dataset) * setting.keep_ratio)

        subset_indices = random.sample(range(len(train_dataset)), num_samples_to_keep)
...
def getfeature_loader(train_dataset: datasets, test_dataset: datasets, setting: IncrementalDataset):
    ...
    if setting.args.featureNet:
        my_file = Path(setting.args.root + "/" + setting.args.dataset + "-" + setting.args.featureNet + "-train-data.npy")
        if my_file.exists():
            print("feature already extracted")
            train_data = np.load(setting.args.root + "/" + setting.args.dataset + "-" + setting.args.featureNet + "-train-data.npy", allow_pickle=True)
            train_label = np.load(setting.args.root + "/" + setting.args.dataset + "-" + setting.args.featureNet + "-train-label.npy", allow_pickle=True)
            test_data = np.load(setting.args.root + "/" + setting.args.dataset + "-" + setting.args.featureNet + "-test-data.npy", allow_pickle=True)
            test_label = np.load(setting.args.root + "/" + setting.args.dataset + "-" + setting.args.featureNet + "-test-label.npy", allow_pickle=True)
        else:
            ...
            np.save(setting.args.root + "/" + setting.args.dataset + "-" + setting.args.featureNet + "-train-data.npy", train_data, allow_pickle=True)
            np.save(setting.args.root + "/" + setting.args.dataset + "-" + setting.args.featureNet + "-train-label.npy", train_label, allow_pickle=True)
            np.save(setting.args.root + "/" + setting.args.dataset + "-" + setting.args.featureNet + "-test-data.npy", test_data, allow_pickle=True)
            np.save(setting.args.root + "/" + setting.args.dataset + "-" + setting.args.featureNet + "-test-label.npy", test_label, allow_pickle=True)
